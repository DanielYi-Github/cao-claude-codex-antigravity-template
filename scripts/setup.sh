...
# pyproject.toml requires Python >=3.12,<3.15 (raised from >=3.11 on
# 2026-08-30: StateDB's migration atomicity fix needs
# sqlite3.Connection.autocommit, a 3.12+ API). Plain `python3` on macOS is
...
if [ -z "$PYTHON" ]; then
  echo "找不到符合 Python >=3.12,<3.15 需求的直譯器（試過 python3.12-3.14 與 python3）。" >&2
